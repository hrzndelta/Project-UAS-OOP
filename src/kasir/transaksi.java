...
int diskonPersen, jumlah, sub, totalHarga, potongan;
        if (jdis.getText().equals("")) {diskonPersen=0;}
        else {diskonPersen= Integer.parseInt(jdis.getText());}
     jumlah= Integer.parseInt(jjum.getText());
     totalHarga = jumlah*iharga;
     potongan = (totalHarga * diskonPersen) / 100;
     sub = totalHarga - potongan;
    ...
    try{
        ...
        pst=koneksi.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);        rst=pst.executeQuery();
        while (rst.next()) {
                if (rst.first() == false) {
                    jkod.setText("TRX001");
                } else {
